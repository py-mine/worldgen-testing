...
palette = {"air": 0, "bedrock": 1, "stone": 2, "dirt": 3, "grass": 4, "water": 5, "diamond_ore": 6, "coal_ore": 7,
    "iron_ore": 8, "gold_ore": 9, "redstone_ore": 10, "lapis_ore": 11, "emerald_ore": 12}
...
def make_ore_pockets(chunks, randomness, noise):
    ...
    for cx, cz in chunks.keys():
        ...
        for v in range(d_veins):
            if (noise.noise2d(cx1+v,cz1+v)/2+0.5)<0.65:
                # diamond ore
                max = (0,0,0,0)
                for y in range(1,17):
                    for z in range(15):
                        for x in range(15):
                            n = noise.noise3d(x+cx1, y, z+cz1)/2+0.5
                            if n>max[3]:
                                max = (x,y,z,n)
                if max[3]>0.85:
        ...
        for v in range(e_veins):
            if (noise.noise2d(cx1+v/2,cz1+v/2)/2+0.5)<0.65:
                # emerald ore
                max = (0,0,0,0)
                for y in range(1,32):
                    for z in range(16):
                        for x in range(16):
                            n = noise.noise3d(x+cx1, y+16*1, z+cz1)/2+0.5
                            if n>max[3]:
                                max = (x,y,z,n)
                if max[3]>0.85:
        ...
        for v in range(g_veins):
            if (noise.noise2d(cx1+v/2,cz1+v/2)/2+0.5)<0.65:
                ...
                for y in range(28,80):
                    for z in range(16):
                        for x in range(16):
                            n = noise.noise3d(x+cx1, y+16*2, z+cz1)/2+0.5
                            if n>max[3]:
                                max = (x,y,z,n)
                if max[3]>0.85:
        ...
        for v in range(l_veins):
            if (noise.noise2d(cx1+v/2,cz1+v/2)/2+0.5)<0.65:
                ...
                for y in range(1,30):
                    for z in range(15):
                        for x in range(15):
                            n = noise.noise3d(x+cx1, y+16*3, z+cz1)/2+0.5
                            if n>max[3]:
                                max = (x,y,z,n)
                if max[3]>0.85:
                    for y in range(l_max_p_dim):
        ...
        for v in range(r_veins):
            if (noise.noise2d(cx1+v/2,cz1+v/2)/2+0.5)<0.65:
                ...
                for y in range(1,15):
                    for z in range(15):
                        for x in range(15):
                            n = noise.noise3d(x+cx1, y+16*4, z+cz1)/2+0.5
                            if n>max[3]:
                                max = (x,y,z,n)
                if max[3]>0.85:
        ...
        for v in range(i_veins):
            if (noise.noise2d(cx1+v/2,cz1+v/2)/2+0.5)<0.65:
                ...
                for y in range(1,63):
                    for z in range(15):
                        for x in range(15):
                            n = noise.noise3d(x+cx1, y+16*5, z+cz1)/2+0.5
                            if n>max[3]:
                                max = (x,y,z,n)
                if max[3]>0.85:
        ...
        for v in range(c_veins):
            if (noise.noise2d(cx1+v/2,cz1+v/2)/2+0.5)<0.65:
                ...
                for y in range(1,63):
                    for z in range(15):
                        for x in range(15):
                            n = noise.noise3d(x+cx1, y+16*6, z+cz1)/2+0.5
                            if n>max[3]:
                                max = (x,y,z,n)
                if max[3]>0.85:
